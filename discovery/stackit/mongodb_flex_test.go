...
type mongoDbFlexSDTestSuite struct {
	Mock *SDMock
}

func (m *mongoDbFlexSDTestSuite) SetupTest(t *testing.T) {
	m.Mock = NewSDMock(t)
	m.Mock.Setup()

	m.Mock.HandleMongoDbFlex()
}

func TestMongoDbFlexSDRefresh(t *testing.T) {
  ...
  "ValidUntil": "2025-04-05T13:34:56Z"
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			d, err := newMongoDBFlexDiscovery(&tc.cfg, promslog.NewNopLogger())
			require.NoError(t, err)
			...
			// one mongo db was not in running state in the mocks
			require.Len(t, targetGroup.Targets, 2)

			for i, labelSet := range []model.LabelSet{
				{
					"__meta_stackit_role":                model.LabelValue(RoleMongoDbFlex),
					"__meta_stackit_mongodb_flex_id":     model.LabelValue("a0e9a075-e485-41ad-b62e-d1c4706a33da"),
					"__meta_stackit_project":             model.LabelValue(testProjectID),
					"__meta_stackit_mongodb_flex_name":   model.LabelValue("instance-1"),
					"__meta_stackit_mongodb_flex_status": model.LabelValue("READY"),
					"__meta_stackit_mongodb_flex_region": model.LabelValue("eu01"),
					"__address__":                        model.LabelValue("mongodb-prom-proxy.api.stackit.cloud"),
